...
def checkPaths(paths, forecast=False):
    ...
    if isinstance(paths, dict):
        if not os.path.isdir(paths['run']):
            raise Exception(
                'Run directory path does not exist: ' +
                paths['run'])
        if not os.path.isdir(paths['obs']):
            raise Exception(
                'Obs directory path does not exist: ' +
                paths['obs'])
        if not os.path.isdir(paths['process']):
            raise Exception(
                'Process directory path does not exist: ' +
                paths['process'])
        if not os.path.isdir(paths['outputs']):
            raise Exception(
                'Outputs directory path does not exist: ' +
                paths['outputs'])
        if not os.path.isdir(paths['images']):
            raise Exception(
                'Images directory path does not exist: ' +
                paths['images'])
    else:
        raise Exception(
            'Must provide path to run directory or specify directories')
