...
def findElementsInRegion(nodalValues, triangleList, targetTag):
    ...
    ix = np.nonzero(
        np.sum(
            nodalValues[triangleList] == targetTag,
            axis=1).flatten() == 3)
    if len(ix) == 0:
        print targetTag
...
def writeGR3File(
    ...
    for i in range(nPoints):
        line = repr(i + 1) + ' ' + string.join(
            nodes[i, :2].astype('|S30'), ' ') + ' ' + repr(nodalValues[i, 0])
        outfile.write(line + '\n')
    if writeTriangles:
        for i in range(ne):
            line = repr(
                i + 1) + ' ' + repr(triType) + ' ' + string.join(triangles[i, :].astype('|S30'), ' ')
            outfile.write(line + '\n')
    # n open bnds
    if boundaries:
        openBnds = []
        ...
        def writeBnds(bndType, bndList):
            ...
            outfile.write(
                '{0:d} = Total number of {1:s} boundary nodes\n'.format(
                    totBndNodes, bndType))
            for i, bnd in enumerate(bndList):
                ...
                # n nodes in bnd 1
                outfile.write(
                    '{n:d} {iTag:s} = Number of nodes for {bTyp:s} boundary {iBnd:d} {tag:s}\n'.format(
                        n=len(
                            bnd.nodes),
                        iTag=islandTag,
                        bTyp=bnd.type,
