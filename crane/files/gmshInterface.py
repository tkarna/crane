...
class gmshMesh(object) :
  ...
  def __init__(self, x,y,z, connectivity, physicalNames=None, lines=None, lineTags=None) :
  ...
  def fromMeshContainer( cls, mc ) :
    ...
    for bnd in bnds :
      ...
      for isEndPoint, p in enumerate([bnd.nodes[0],bnd.nodes[-1]]) :
          ...
          if not connected :
            ...
            candList = []
            for ob in other_bnds :
              for isEndPoint2, q in enumerate([ ob.nodes[0], ob.nodes[-1] ]) :
                p2 = bnd.nodes[0] if isEndPoint else bnd.nodes[-1]
                if q != p2 :
                  d = np.sqrt( (mc.x[p]-mc.x[q])**2 + (mc.y[p]-mc.y[q])**2 +
                              (mc.z[p]-mc.z[q])**2 )
                  distList.append( d )
    ...
    return cls( mc.x,mc.y,mc.z, mc.connectivity, physicalNames, lines, lineTags)
  ...
  def writeMesh(self, outfilename, verbose=False) :
    ...
    tmp = np.hstack( (np.arange(1,nTriangles+1)[:,None]+nLines,
                      ...
                      np.ones((nTriangles,1),dtype=int)*elemTag, # physical entity
                      np.ones((nTriangles,1),dtype=int)*elemTag, # geometrical entity
                      self.connectivity+1) )
    np.savetxt(outfile,tmp,fmt='%d')
    for i in range(nPrisms) :
      ...
      nodes = prismList[i][1:]
      outfile.write( '{i:d} {type:d} {ntags:d} '.format(i=iElem,type=elemType,ntags=nTags) )
      for tag in tags :
  ...
  def getBoundaries(self) :
    if self.lines != None and len(self.physicalNames) > 0 :
  ...
  def getMeshContainer( self, nodalValues=None, fieldNames=None, fromFile=None ) :
    ...
    if nodalValues == None :
      try :
        ...
        try :
          ...
          elemValues,fieldName,time,it,nScalars = self.readElemValues(fromFile)
          # convert to continuos by taking a node (by averaging around nodes)
    ...
    mc = meshContainer( description, ta,self.x,self.y,self.z,
                        data,self.connectivity,fieldNames,coordSys='spcs')
  ...
  def writeNodalDataFromMC(self,outfilename,mc,iField=0,append=False,
    ...
    if not np.array_equal( mc.connectivity, self.connectivity ) :
      raise Exception( 'Given MeshContainer not compatible with this mesh.')
    time=0
    iteration=0
  ...
  def writeNodalData(self,outfilename,values,fieldName,
    ...
    if values.shape[0] > 0 :
      ...
      outfile.write( '{0:d}\n'.format( iteration ) ) # iteration / export count
      outfile.write( '{0:d}\n'.format( nScalars ) ) #   number of fields per node
      outfile.write( '{0:d}\n'.format(nNodes) ) # number of nodes with values
  ...
  def writeElemData(self,outfilename,values,fieldName,
    ...
    if values.shape[0] > 0 :
      ...
      outfile.write( '{0:d}\n'.format( iteration ) ) # iteration / export count
      outfile.write( '{0:d}\n'.format( nScalars ) ) #   number of fields per node
      outfile.write( '{0:d}\n'.format(nElems) ) # number of elements with values
      num = np.arange(nElems)[:,None] + 1 + nLines
      tmp = np.hstack( (num,np.ones_like(num)*nNodes*nScalars,values) )
      fmt = ['%d','%d']
  ...
  def readElemValues(self,filename) :
    ...
    if self.lines != None :
      ...
      nLines = 0
    nb = np.fromfile(infile,dtype=int,count=1,sep=' ')[0] # nb of str tags
    fieldName = infile.readline().strip().strip('\"') # field name
    if nb > 1 :
      for i in range(1,nb) :
        infile.readline().strip().strip('\"') # other tags
    nb = np.fromfile(infile,dtype=int,count=1,sep=' ')[0] # nb of float tags
    time = np.fromfile(infile,dtype=float,count=1,sep=' ')[0] # time step
    nb = np.fromfile(infile,dtype=int,count=1,sep=' ')[0] # nb of int tags
    it = np.fromfile(infile,dtype=int,count=1,sep=' ')[0] # iteration
    nScalars = np.fromfile(infile,dtype=int,count=1,sep=' ')[0] # nb scalars per node
    nTriangles = np.fromfile(infile,dtype=int,count=1,sep=' ')[0] # nb elems with values
  ...
  def readNodalValues(self,filename) :
    ...
    while True :
      line = infile.readline().strip()
      if line == '' : # end of file
        raise Exception('$NodeData section not found in file '+filename)
      if line == '$NodeData' :
        break
    ...
    it = np.fromfile(infile,dtype=int,count=1,sep=' ')[0] # iteration
    nScalars = np.fromfile(infile,dtype=int,count=1,sep=' ')[0] # nb scalars per node
    nNodes = np.fromfile(infile,dtype=int,count=1,sep=' ')[0] # nb nodes with values
  ...
  def writePosFileFromMC( self, outfilename, mc, iField=0, append=False ) :
    self.writePosFile( outfilename, mc.data[:,iField,0], mc.fieldNames[0], append )

  def writePosFile( self, outfilename, nodalValues, fieldName='field', append=False ) :
...
def findBoundarySegments( lines, lineTags, physicalNames ) :
  ...
  # find open bnd tags
  openTag = [ n for n in tags if physicalNames[(lineDim,n)][:4] not in landNames ]
  tags = list(openTag)
  tags.extend(landTag)
    ...
    for i,lineNodes in enumerate(lines) :
      ...
      for s in segments :
        if s[0].nodes[0] in L.nodes :
          ...
          parentSegment = s
          break
      if parentSegment != None :
        # check for merged segments
        nodes = [parentSegment[0].nodes[0], parentSegment[-1].nodes[-1]]
        merged = False
        for s in segments :
          if s == parentSegment :
            continue
  ...
  for tag in tags :
    ...
    bndType = 'land'
    if 'island' in tagStr : bndType = 'island'
    if 'open' in tagStr : bndType = 'open'
