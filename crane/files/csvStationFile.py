...
class csvStationFile(collection.tupleList) :
  ...
  def readFromFile(self,filename, verbose=False):
    ...
    with open(filename, 'r') as f:
      reader = csv.reader(filter(lambda row: row[0]!='#', f),delimiter=',',skipinitialspace=True)
      for line in reader:
        try :
          ...
          print e
          raise Exception('Malformed line '+str(line)+' in file '+filename )
    return self
...
class csvStationFileWithDepth(csvStationFile) :
  ...
  def sort(self) :
    """Sorts the tuples in place."""
    self.data = sorted(self.data, key=lambda o: o[self.keywords.index('z')])
    self.data = sorted(self.data,
                       key=lambda o: o[self.keywords.index('location')])
    self.data = sorted(self.data,
