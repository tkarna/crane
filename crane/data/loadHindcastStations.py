        ...
        if vars3d:
            for i in range(self.nRec):
                ...
                if len(zz) > 0:
                    ...
                    # interpolate
                    for var in vars3d:
                        outData[var][i] = interp1d(
                            zz, self.data[var][i, incr_ix])(zInterp)
...
def getWeeksAndDays(startTime, endTime):
    ...
    ordinalDays = np.arange(
        startTime.toordinal(),
        endTime.toordinal() +
        1)  # unique int
    years = np.array([datetime.datetime.fromordinal(d).year
                      for d in ordinalDays])
    # day of year
    yearDays = ordinalDays - np.array(
        [datetime.datetime(year, 1, 1).toordinal() for year in years]) + 1
    # corresponding week (1st week starts Jan 1st)
    weeks = np.floor((yearDays - 1) / 7) + 1
...
def readDateRange(
    ...
    ordinalDays, years, yearDays, weeks, weekDays = getWeeksAndDays(
        startTime, endTime)
    ...
    for i in range(len(ordinalDays)):
        ...
        dataDir = getWeeklyDataDir(
            baseDir, years[i],
            weeks[i],
            tagStr, subdir='process')
        filename = '{s:s}_elcirc_n{n:03d}.dat'.format(
            s=station, n=int(weekDays[i]))
    ...
    for i in range(len(time) - 1):
        if time[i][-1] > time[i + 1][0]:
            ...
            raise Exception(
                'overlapping days... something is wrong with the time stamps')
    ...
    for var, zCoord, bracket in data:
        ...
        dc = dataContainer.dataContainer(
            '', ta, stationX, stationY, zCoord, di, [var], coordSys, meta)
        dcList.append(dc)
