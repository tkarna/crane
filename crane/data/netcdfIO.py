...
class netcdfIO(object) :
  ...
  def readHeader( self, startTime=None, endTime=None, verbose=True ) :
    ...
    return netcdfIO.readNetCDFFile( ncfile, startTime, endTime, headerOnly=True )

  def read( self, startTime=None, endTime=None, includeEnd=False, verbose=True ) :
  ...
  def saveDataContainer( self, dataContainer, dtype=np.float64, overwrite=False,
    ...
    if os.path.exists(self.filename) :
      if os.path.isfile(self.filename) :
        if not overwrite :
          raise IOError('file already exists: {0:s}'.format(self.filename) )
        else :
          print ' file exists, overwriting ...'
          os.remove(self.filename)
      else :
        raise IOError('unable to create file: {0:s}'.format(self.filename) )
    createDirectory(os.path.split(self.filename)[0])
    ncfile = NetCDFFile(self.filename,'w')
    self.populateNetCDFObject( ncfile, dataContainer, dtype, compress, digits )
    # close the file.
    ncfile.close()
  ...
  def readNetCDFFile( ncfile, startTime=None, endTime=None, headerOnly=False,
    ...
    nX = len(ncfile.dimensions['x']) if 'x' in ncfile.dimensions.keys() else 1
    nY = len(ncfile.dimensions['y']) if 'y' in ncfile.dimensions.keys() else 1
    nZ = len(ncfile.dimensions['z']) if 'z' in ncfile.dimensions.keys() else 1
    nData = len(ncfile.dimensions['data']) if 'data' in ncfile.dimensions.keys() else nX
  ...
  def populateNetCDFObject( self, ncfile, dc, dtype=None, compress=False, digits=None ) :
    ...
    if hasattr(dc, 'connectivity') :
      ...
      connVar = createVariable(ncfile,'connectivity',np.dtype(np.int64).char,('nElements','nElemNodes',),compress)
    ...
    if hasattr(dc, 'boundaries') :
      ...
      for i,bnd in enumerate(dc.boundaries,start=1) :
        ...
        bndVar = createVariable(ncfile,bndNameStr,np.dtype(np.int64).char,(bndDimStr,),compress)
