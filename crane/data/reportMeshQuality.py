...
def parseCommandLine():
    ...
    parser.add_option(
        ...
        type='float',
        dest='maxAngle',
        help='Define the maximum angle in triangle to classify element as acceptable (e.g. 160)')
    parser.add_option(
        '-q',
        '--minQuality',
        action='store',
        type='float',
        dest='minQuality',
        help='Define minimum element quality to classify element as acceptable (e.g. 0.05)')
    parser.add_option(
        '-O',
        '--one-based-index',
        action='store_true',
        default=False,
        dest='oneBasedIndex',
        help='Report element indices starting from 1 (e.g. in gr3 file) instead of 0 (Default: %default)')
