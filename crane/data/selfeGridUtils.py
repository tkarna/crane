...
class verticalCoordinates :
  ...
  def computeVerticalCoordinates2(self, eta, dp, z=None, kbp=None,
    ...
    selfeVerticalCoords.vertCoords(eta, dp, z, kbp, iwet, self.ztot, self.cs,
                                   self.sigma, self.nz, self.h0,
                                   self.h_c, self.h_s)
...
def findTriangle( tree, node2elem, triX, triY, conn, x, y, verbose=True ) :
  ...
  if len(it) == 0 :
    if verbose : print 'parent triangle not found'
...
def findStations( tree, node2elem, triX, triY, conn, staX, staY, stationNames=None, verbose=True ) :
  ...
  for i in range(nSta) :
    nn,it,uu = findTriangle( tree, node2elem,triX,triY,conn,staX[i],staY[i], verbose )
    if it != None :
      iTri[i] = it
      u[i,:] = uu
      n[i,:] = nn
      ...
      if verbose : print 'station out of grid, skipping:',sta,staX[i],staY[i]
