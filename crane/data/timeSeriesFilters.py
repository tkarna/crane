...
def runningX(dc, T=T_M2, operator=computeRunningMean, gap_dt=None):
    ...
    for p in range(nPoints):
        for f in range(nFields):
            ...
            if p == 0 and f == 0:
                ...
                nNewTime = len(vRes)
                data = np.empty((nPoints, nFields, nNewTime))
                xout = np.empty((nPoints, nNewTime)) if dc.xDependsOnTime else dc.x
                yout = np.empty((nPoints, nNewTime)) if dc.yDependsOnTime else dc.y
                zout = np.empty((nPoints, nNewTime)) if dc.zDependsOnTime else dc.z
                meta = dc.getMetaData()
            if dc.xDependsOnTime:
                xout[p, :] = xRes
            if dc.yDependsOnTime:
    ...
    dc2 = dataContainer.dataContainer('', ta, xout, yout, zout, data, dc.fieldNames,
                                      coordSys=dc.coordSys, metaData=meta,
                                      acceptNaNs=True)
    return dc2
