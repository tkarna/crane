...
def getAvailableOfferings( startTime, endTime, variables=['elev','temp','salt'] ) :
...
class netcdfCacheReader(object) :
  ...
  def getData(self, startTime, endTime, quality='best') :
    ...
    (time, y, units) = getncdatastation(self.station, self.nc_offering, startEpoch,
                                  endEpoch, vars=[self.var], quality=quality)
    if len(time) == 0 :
    ...
    y = y[valid]
    if len(time) == 0 :
      raise Exception('Retrieving data failed after quality check '+self.offerString)
...
def getDataContainerFromOffering( offeringDict, startTime, endTime, quality='best' ) :
...
def getncdatastation(station, offering,starttime,endtime,vars=['all'],quality='best',basedir=DEF_BASEDIR):
  if quality == 'best':
  ...
  agg_units = {}
  agg_time = np.array([])
  cf = dict([["salt", "salinity"],["temp", "temperature"],["cond", "electrical conductivity"],["elev", "elevation"],["pres", "pressure"],["fluores", "fluorescence"]])
  ...
  for year in range(int(startyear),int(endyear)+1):
     ...
     for month in range(sm,em+1):
        try:
          ...
          #   tmp_time = tmp_time[time_ind]
          #   cmop.debug('reduced to %d values by time restriction' % (len(tmp_time)))
          #else: cmop.debug('(%s %s) "%s" != "%s" or "%s" != "%s" and %s != %s or %s != %s' % ((year==int(startyear)),(sm==month),year, startyear, sm, month,year, endyear, em,month))
          ...
          if len(varmap) != len(vars):
             ...
             raise DataError( 'data file %s is missing expected variables: %s present, %s expected' % (file,', '.join(varmap),', '.join(vars)) )
          tmp_vars = agg_vars
          ...
          for var in varmap:
              ...
              if len(values) != len(tmp_time):
                 ...
                 raise DataError('skipping data file %s, length of variable %s does not match length of time' %
                                 (file,var) )
...
if __name__=='__main__' :
  ...
  sT = datetime.datetime(2011,05,14)
  eT = datetime.datetime(2011,07,20)
