...
def getSurrogateModelDiff(surTag, modTag, location, z=None, k=None):
    ...
    tagStr = sur.getMetaData('tag').split(
        '-')[0] + '-' + mod.getMetaData('tag').split('-')[0]
...
def printStatsLegend(fid, metrics):
    ...
    for dataSetName, measure in metrics:
        setAbbrev = '' if dataSetName in [
            'original',
            'all'] else dataSetName[
            :2] + '_'
...
def processStats(refTag, runTags, stationFile, startTime=None, endTime=None,
    ...
    stationStats = stationCollection.tinyDB(
        ['tag', 'location', 'variable', 'msldepth'])
    ...
    allData = {}
    murphyScores = stationCollection.tinyDB(
        ['tag', 'location', 'variable', 'msldepth'])
    # for loc,x,y,z,zType,var in stations.getTuples() :
    ...
    for filt in stationFilter:
        ...
        try:
            ...
            ref = dirTreeManager.getDataContainer(
                tag=refTag, location=loc, dataType='timeseries',
                msldepth=msldepth, variable=var, rule=defRule,
                startTime=startTime, endTime=endTime)
            stationStats.addSample(
                getStats(
                    ref,
                    ref),
                tag=refTag,
            ...
            for runTag in runTags:
                try:
                    sig = dirTreeManager.getDataContainer(
                        tag=runTag, location=loc, dataType='timeseries',
                        msldepth=msldepth, variable=var, rule=defRule,
                        startTime=startTime, endTime=endTime)
                    stationStats.addSample(
                        getStats(ref, sig),
                        tag=runTag, location=loc, variable=var,
                        msldepth=msldepth)
            ...
            if len(modSignals) == 2:
                ...
                st = max(
                    ref.time.getDatetime(0),
                    sig1.time.getDatetime(0),
                    sig2.time.getDatetime(0))
                et = min(
                    ref.time.getDatetime(-1),
                    sig1.time.getDatetime(-1),
                    sig2.time.getDatetime(-1))
                if st > et:
                ...
                st2 = max(
                    r.time.getDatetime(0),
                    s1.time.getDatetime(0),
                    s2.time.getDatetime(0))
                et2 = min(
                    r.time.getDatetime(-1),
                    s1.time.getDatetime(-1),
                    s2.time.getDatetime(-1))
                ...
                s = {'original': {}}
                s['original']['MS'] = mur
                murphyScores.addSample(
                    s, tag=runTags[0] + ' vs ' + runTags[1],
...
def parseCommandLine():
    ...
    parser.add_option(
        ...
        type='string',
        dest='refTag',
        help='Name of data set to use as reference')
    ...
    parser.add_option(
        ...
        type='string',
        dest='startStr',
        help='Date to start processing (optional)')
    ...
    parser.add_option(
        ...
        type='string',
        dest='outFile',
        help='File where statistics are stored (optional, default:print to stdout)')
