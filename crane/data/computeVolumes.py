...
class volumeComputer(object):
    ...
    def createRegionMesh(self, regionMask):
        ...
        goodElems = np.alltrue(
            regionMask[
                self.volumeNCReader.faceNodes],
            axis=1)
        self.region_mask = np.sort(
            np.unique(
                self.volumeNCReader.faceNodes[
                    goodElems, :]))
        ...
        self.nodesToMiniMesh = -1 * np.ones_like(
            self.volumeNCReader.nodeX, dtype=int)
        # print self.nodeX.shape
    ...
    def calculateVolume(self, timeStep, volFile, criteriaFile=None):
        ...
        if self.volumeFileType == '63':
            region_stats = self.calculateNodalVolume(
                timeStep, volFile, criteriaFile)
        elif self.volumeFileType == '70':
            region_stats = self.calculateElementVolume(
                timeStep, volFile, criteriaFile)
        else:
            raise Exception(
                'Given GR3 file does not match the mesh in output files')
    ...
    def calculateNodalVolume(self, timeStep, volFile, criteriaFile):
        ...
        if self.regionFile is not None:
            volVar = volFile.variables[
                self.volumeVar][
                timeStep, :, :][
                :, self.region_mask]
            eta = volFile.variables['elev'][timeStep, :][self.region_mask]
            if criteriaFile is not None:
                criteria_var = criteriaFile.variables[
                    self.criteriaVar][
                    timeStep, :, :][
                    :, self.region_mask]
        else:
        ...
        # Prepare vertical
        Z, kbp2, iwet = self.volumeNCReader.vCoords.computeVerticalCoordinates(
            eta, self.bath)
        ...
        region_x = np.sum((self.areas * self.elem_center_x)
                          [region_triangles]) / region_area
        region_y = np.sum((self.areas * self.elem_center_y)
                          [region_triangles]) / region_area
        region_var_nodes = np.ma.array(volVar, mask=(
            np.tile(~region_nodes, (volVar.shape[0], 1))))
        ...
        for i in np.arange(nLevels):
            region_var_tris[
                ...
                self.faceNodes].mean(
                axis=1)
        region_var_tris = np.ma.masked_where(
            region_var_tris == 1.0000002e+20, region_var_tris)
        ...
        region_height_elems = (
            region_height_tris_top + region_height_tris_bot) / 2.0
        region_height_tri_total = region_height_nodes_total[
            self.faceNodes].mean(axis=1)
...
# - One node can be wet to include element
        ...
        region_conc = self.areas * region_height_tris * \
            volVar[1:, :] * vol_in_range[1:, :]
        region_conc_total = region_conc.sum()
        if region_area > 0:
            region_volume = self.areas * \
                region_height_tris * vol_in_range[1:, :]
            #region_volume = region_volume[region_triangles]
            region_volume = region_volume.sum()
            region_thickness = region_volume / region_area
        else:
    ...
    def processStack(self, stack):
        ...
        for iTime in xrange(nTime):
            region_stats[
                iTime, :] = self.calculateVolume(
                iTime, ncfile, criteria_file)
    ...
    def getDataContainer(self, startTime, endTime):
        ...
        for i, var in enumerate(varNames):
            ...
            region_desc = [
                self.volumeVar,
                self.volumeThresholdLow,
                self.volumeThresholdHi,
                self.criteriaVar,
                self.criteriaThresholdLow,
                self.criteriaThresholdHi]
...
def parseCommandLine():
    ...
    parser.add_option(
        ...
        type='string',
        dest='volFile',
        help='File with variable to calculate volume')
    parser.add_option(
        '-v',
        '--volumeVar',
        action='store',
        type='string',
        dest='volVar',
        help='Variable in volume file to calculate volume')
    parser.add_option(
        '-l',
        '--volThreshLow',
        action='store',
        type='float',
        dest='volThreshLow',
        help='Lower bound of value to describe region to calculate volume',
        default=None)
    parser.add_option(
        '-u',
        '--volThreshHi',
        ...
        type='float',
        dest='volThreshHi',
        help='Upper bound of value to describe region to calculate volume',
        default=None)
    parser.add_option(
        '-F',
        '--criteriaFile',
        ...
        type='string',
        dest='criFile',
        help='File with variable to constrain calculation of volume',
        default=None)
    parser.add_option(
        '-V',
        '--criVar',
        ...
        type='string',
        dest='criVar',
        help='Variable in criteria file to calculate volume',
        default=None)
    parser.add_option(
        '-L',
        '--criThreshLow',
        ...
        type='float',
        dest='criThreshLow',
        help='Lower bound of criteria value to describe region to calculate volume',
        default=None)
    parser.add_option(
        '-U',
        '--criThreshHi',
        ...
        type='float',
        dest='criThreshHi',
        help='Upper bound of criterita value to describe region to calculate volume',
        default=None)
    parser.add_option(
        '-t',
        '--regionRegionFile',
        action='store',
        type='string',
        dest='regionRegionGR3File',
        help='a gr3 file indicating the region region (depth==1)',
        default=None)
    parser.add_option(
        '-n',
        '--regionName',
