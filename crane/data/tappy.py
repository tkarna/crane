...
class Util():
    ...
    def sum_signals(self, skey_list, hours, speed_dict, amp=None, phase=None):
        ...
        for i in skey_list:
            ...
            component = R * speed_dict[i]['FF'] * np.cos(
                speed_dict[i]['speed'] * hours - (p - speed_dict[i]['VAU']) * deg2rad)
...
#        self.tidal_dict["OQ2"] =
        ...
        (zetatoss,
         ...
         Nvbeg,
         pbeg,
         p1beg) = self.astronomic([self.dates[0],
                                   self.dates[0] + datetime.timedelta(hours=1)])
        Tspeed = 15.0 * deg2rad
        ...
        w = np.array([(Tspeed - sspeed + hspeed), sspeed, hspeed,
                      pspeed, Nvspeed, ppspeed, 0.0]) * rad2deg
        ...
        for key in self.tidal_dict:
            ...
            self.tidal_dict[key]['speed'] = np.sum(
                np.array(constituent_factors) * w)
            self.tidal_dict[key]['speed'] = np.mod(
                self.tidal_dict[key]['speed'], 360)
            self.tidal_dict[key]['speed'] = self.tidal_dict[
                key]['speed'] * deg2rad

            self.tidal_dict[key]['V'] = np.sum(
                np.array(constituent_factors) * vw1)
            ...
            try:
                self.tidal_dict[key]['VAU'] = np.mod(
                    self.tidal_dict[key]['VAU'], 360)[0]
            except IndexError:
                self.tidal_dict[key]['VAU'] = np.mod(
                    self.tidal_dict[key]['VAU'], 360)
        ...
        if num_hours < 13:
            ...
            # sys.exit()
            raise Exception(
                "Cannot calculate any constituents from this record length")
...
#            speed_dict["H2"] =self.tidal_dict["H2"]
        ...
        key_list = sorted(speed_dict.keys())
...
class tappy(Util):
    ...
    def open(self, filename, def_filename=None):
        ...
        if len(self.elevation) == 0:
            ...
            # sys.exit() # HACK
            raise Exception('No data was found in the input file.')
        self.elevation = np.array(self.elevation)
        self.dates = np.array(self.dates)

    # HACK
    def setElevation(self, dates, elevation):
    ...
    def missing(self, task, dates, elev):
        ...
        if np.any(interval > datetime.timedelta(seconds=3600)):
            if task == 'fail':
                ...
                # sys.exit() # HACK
                raise Exception(
                    "There is a difference of greater than one hour between values")
        ...
        if task == 'fill':
            ...
            # self.constituents()
            total = self.sum_signals(
                self.key_list, dates_filled, self.speed_dict)
    ...
    def residuals(self, p, ht, t, key_list):
        ...
        if self.include_inferred:
            if 'O1' in key_list and 'K1' in key_list:
                if 'J1' not in key_list:
                    ...
                    self.inferred_phase['J1'] = phase['J1'] = phase[
                        'K1'] + 0.496 * (phase['K1'] - phase['O1'])
                # How should I handle this?  Shureman seems to confuse M1 and
                # NO1
                if 'M1' not in key_list:
                    self.inferred_key_list.append('M1')
                    self.inferred_r['M1'] = H['M1'] = 0.071 * H['O1']
                    self.inferred_phase['M1'] = phase['M1'] = phase[
                        'K1'] - 0.5 * (phase['K1'] - phase['O1'])
                if 'OO1' not in key_list:
                    self.inferred_key_list.append('OO1')
                    self.inferred_r['OO1'] = H['OO1'] = 0.043 * H['O1']
                    self.inferred_phase['OO1'] = phase['OO1'] = phase[
                        'K1'] + 1.0 * (phase['K1'] - phase['O1'])
                if 'P1' not in key_list:
                    ...
                    self.inferred_phase['P1'] = phase['P1'] = phase[
                        'K1'] - 0.075 * (phase['K1'] - phase['O1'])
                if 'Q1' not in key_list:
                    self.inferred_key_list.append('Q1')
                    self.inferred_r['Q1'] = H['Q1'] = 0.194 * H['O1']
                    self.inferred_phase['Q1'] = phase['Q1'] = phase[
                        'K1'] - 1.496 * (phase['K1'] - phase['O1'])
                if '2Q1' not in key_list:
                    self.inferred_key_list.append('2Q1')
                    self.inferred_r['2Q1'] = H['2Q1'] = 0.026 * H['O1']
                    self.inferred_phase['2Q1'] = phase['2Q1'] = phase[
                        'K1'] - 1.992 * (phase['K1'] - phase['O1'])
            ...
            for key in self.inferred_key_list:
                ...
                try:
                    self.inferred_phase[key] = np.mod(
                        self.inferred_phase[key] + self.tidal_dict[key]['VAU'], 360)[0]
                except IndexError:
                    self.inferred_phase[key] = np.mod(
        ...
        for i in key_list + self.inferred_key_list:
            sumterm = sumterm + H[i] * ff[i]['FF'] * np.cos(
    ...
    def constituents(self):
        ...
        if np.any(difference < datetime.timedelta(seconds=0)):
            ...
            # sys.exit() # HACK
            raise Exception(
                "The date values reverse - they must be constantly increasing.")
        ...
        for index, key in enumerate(self.key_list):
            ...
            try:
                self.phase[key] = np.mod(
                    self.phase[key] + self.speed_dict[key]['VAU'], 360)[0]
            except IndexError:
                self.phase[key] = np.mod(
    ...
    def cat_dates(self, dates, len_dates):
        ...
        interval = interval[len(interval) / 2]
        cnt = np.arange(
            1, len_dates + 1) * datetime.timedelta(minutes=interval.seconds / 60)
        bdate = dates[0] - cnt[::-1]
        edate = dates[-1] + cnt
    ...
    def filters(self, nstype, dates, elevation, pad_type=None):
        ...
        # For the time being the filters and padding can only work on hourly
        # data.
        ...
        dates_filled, nelevation = self.missing(
            'fill', dates_filled, nelevation)
        ...
        if nstype == 'transform':
            ...
            """
            import filter
            return dates_filled, filter.fft_lowpass(
                nelevation, 1 / 30.0, 1 / 40.0)
        ...
        if nstype == 'lecolazet1':
            ...
            relevation = 1.0 / 16.0 * (self.delta_diff(nelevation, 24, 25)
                                       [25:] * self.delta_diff(nelevation, 25, 25)[25:])**2
        ...
        if nstype == 'mstha':
            ...
            new_dates = np.concatenate(
                ([ndates[0] - datetime.timedelta(hours=blen)],
                 ndates, [ndates[-1] + datetime.timedelta(hours=blen)]))
            new_elevation = np.concatenate(([nelevation[0]],
                                            nelevation,
                                            [nelevation[-1]]))
            (new_dates, new_elev) = self.missing(
                'fill', new_dates, new_elevation)
            slope = []
        ...
        if nstype == 'wavelet':
            ...
            for wl in pywt.wavelist():
                ...
                for index, items in enumerate(a):
                    self.write_file(
                        dates, items, fname="outts_wavelet_%s_%i.dat" %
                        (wl, index))
        ...
        if nstype == 'cd':
            ...
            for key in self.key_list:
                ...
                yt = new_elev * np.exp(-1j * self.speed_dict[key][
                                       'speed'] * ntimes_filled)
    ...
    def sortbyvalue(self, dict):
        ...
        mdict = sorted(map(_swap2, dict.items()))
    ...
    def print_ephemeris_table(self):
        ...
        for d in range(1600, 2001, 100):
            ...
            (zeta, nu, nup, nupp, kap_p, ii, R, Q,
             T, self.jd, s, h, Nv, p, p1) = package
            Ra = 1.0 / np.sqrt(1.0 - 12.0 * (np.tan(0.5 * ii))**2 * np.cos(
                2.0 * kap_p) + 36.0 * (np.tan(0.5 * ii))**4)  # eq 215, schureman
...
def prediction(
    ...
    end_date = datetime.datetime.strptime(
        end_date, '%Y-%m-%dT%H:%M:%S') + datetime.timedelta(minutes=1)
...
def analysis(
    ...
    (x.zeta, x.nu, x.nup, x.nupp, x.kap_p, x.ii, x.R,
     x.Q, x.T, x.jd, x.s, x.h, x.N, x.p, x.p1) = package
    ...
    if x.filter:
        for item in x.filter.split(','):
            if item in [
                    ...
                    'lecolazet1',
                    'kalman',
                    'transform']:  # 'lecolazet', 'godin', 'sfa']:
                filtered_dates, result = x.filters(item, x.dates, x.elevation)
                x.write_file(
                    filtered_dates, result, fname='outts_filtered_%s.dat' %
                    (item,))
        (x.speed_dict, x.key_list) = x.which_constituents(len(x.dates),
                                                          package,
                                                          rayleigh_comp=ray)
