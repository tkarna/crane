...
class dataContainer(object):
  ...
  def fromTimeSeries(cls, description, time, data, fieldNames,
    ...
    if len(data.shape) > 1 :
      ...
      if data.shape[0] == nTime :
        ...
        raise Exception( 'data array size does not match time series specs' )
    ...
    return cls(description, time, x,y,z, data, fieldNames, coordSys, metaData)
    
  def copy(self) :
    """Deep copy, all numpy arrays are copied instead of referenced."""
    return dataContainer(self.description, self.time.copy(),
                         self.x.copy(), self.y.copy(), self.z.copy(),
                         self.data.copy(), list(self.fieldNames), self.coordSys,
                         metaData=dict(self.metaData),acceptNaNs=self.acceptNaNs,
                         dtype=self.dtype)
  ...
  def xyzMatch( self, other ) :
    ...
    if ( np.isnan(self.x).all() and np.isnan(other.x).all() and
      ...
      return ( np.array_equal( np.nan_to_num(self.x), np.nan_to_num(other.x) ) and
  ...
  def interpolateInTime(self, newTime, acceptNaNs=False) :
    ...
    return dataContainer(self.description, newTime, newx, newy, newz, newdata,
                         list(self.fieldNames), self.coordSys, dict(self.metaData), acceptNaNs=acceptNaNs)
  ...
  def subsample( self, timeStamps=None, skipFactor=None, targetDt=None, currentDt=None, gapFactor=5) :
    ...
    return dataContainer(self.description,newt, x,y,z,
                         newData,self.fieldNames,self.coordSys, self.metaData,
                         acceptNaNs=True, checkDataXDim=False)
  ...
  def alignTimes( self, other ) :
    ...
    if (other.getMetaData('location',suppressError=True) == 'saturn03' and
      ...
      otherDt = 360
    oTimeStamps = self.time.getAlignedTimeIndices( other.time,selfDt,otherDt )
    ...
    newOther = dataContainer(other.description, newTA, x,y,z,
                          other.data[:,:,oTimeStamps], list(other.fieldNames), other.coordSys, other.metaData)
...
def arrayRangeStr(fieldName,array) :
  ...
  if N == 1 :
    ...
    outstr = tmpStr.format(name=fieldName,shape=array.shape,v=float(uarray[0]))
  else :
    uarray = uarray[ np.logical_not( np.isnan(uarray) ) ]
    tmpStr = '{name:s}: {shape:s} range: {m:g} ... {M:g}'
    outstr = tmpStr.format(name=fieldName,shape=array.shape,
                        m=float(np.min(uarray)),M=float(np.max(uarray)))
  return outstr
