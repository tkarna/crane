...
def nominator( S, Th, p ) :
  ...
  Pn = ( a[0] + Th*a[1] + Th*Th*a[2] + Th*Th*Th*a[3] + S*a[4] + Th*S*a[5] +
         S*S*a[6] + p*a[7] + p*Th * Th*a[8] + p*S*a[9] + p*p*a[10] +
         p*p*Th*Th*a[11] )
  return Pn

def denominator( S, Th, p ) :
  """Computes the denominator of the equation of state."""
  Pd = ( b[0] + Th*b[1] + Th*Th*b[2] + Th*Th*Th*b[3] + Th*Th*Th*Th*b[4] +
         S*b[5] + S*Th*b[6] + S*Th*Th*Th*b[7] + pow(S,1.5)*b[8] +
         pow(S,1.5)*Th*Th*b[9] + p*b[10] + p*p*Th*Th*Th*b[11] + p*p*p*Th*b[12] )
  return Pd
...
def equationOfStateJackettAlpha(S, Th, p) :
  ...
  Pn = nominator(S, Th, p)
  dndT = a[1] + 2*a[2]*Th +3*a[3]*Th*Th + a[5]*S + 2*a[8]*p*Th + 2*a[11]*p*p*Th
  Pd = denominator(S, Th, p)
  dddT = ( b[1] + 2*b[2]*Th + 3*b[3]*Th*Th + 4*b[4]*Th*Th*Th + b[6]*S +
           3*b[7]*S*Th*Th + 2*b[9]*pow(S,1.5)*Th + 3*b[11]*p*p*Th*Th +
           b[12]*p*p*p )
