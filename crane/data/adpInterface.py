...
class netcdfCacheADPReader(netcdfCacheInterface.netcdfCacheReader):
    ...
    def getData(self, starttime, endtime):
        ...
        t, v, u = netcdfCacheInterface.getncdatastation(
            self.station, self.nc_offering, starttime, endtime, variables, quality='PD0')

        if t.shape[0] == 0:
            raise Exception(
                "Empty files found for %s - %s time range" %
                (timeMod.gmtime(starttime), timeMod.gmtime(endtime)))
...
def getADPData(offering, sT, eT, var):
    ...
    # convert datetime to epochtime with timeArray methods (safe timezone
    # convert)
    ...
    return dataContainer.dataContainer(
        '', ta, xv, yv, zv, data, [var],
        coordSys=sta.coordSys, acceptNaNs=True, metaData=meta)
...
def parseCommandLine():
    ...
    parser.add_option(
        ...
        type='string',
        dest='var',
        help='variable to extract: alongvel, crossvel, vel_n, vel_vert, vel_mag,, bindist, vel_e, bs_avg')
    parser.add_option(
        '-o',
        '--outDirectory',
        action='store',
