...
def datetime2matlabdn(dt):
    ...
    frac_seconds = (dt - datetime.datetime(dt.year, dt.month,
                                           dt.day, 0, 0, 0)).seconds / (24.0 * 60.0 * 60.0)
...
def genSILDC(path, fn, runTag):
    ...
    meta['variable'] = var
    meta['tag'] = runTag
    goodIx = np.logical_and(
        np.isfinite(time), np.any(
            np.isfinite(vals), axis=0))
...
def getTSFromProfile(runTag, location, var, z=None, k=None,
    ...
    """
    if var == 'elev':
        return dirTreeManager.getDataContainer(
            location=location,
            tag=runTag,
            dataType='timeseries',
            variable=var,
    ...
    prof = dirTreeManager.getDataContainer(
        location=location,
        tag=runTag,
        dataType='profile',
        variable=var,
    ...
    for iComp in range(nComp):
        v0, z_target = interpolateInVertical(
            prof.z, prof.data[:, iComp, :], z, k)
        #z = z_target.mean()
        v.append(v0)
    v = np.vstack(tuple(v))
    # make dataContainer
...
def read_mat_plume_file(path, var, saltThreshold):
    ...
    f = os.path.join(path, varToChar[var] +
                     '_ms_' +
                     str(int(saltThreshold)) +
                     '.mat')
...
def gatherProfiles(
    ...
    dcs = []
    for var in varList:
        varStations = collection.uniqueList(
            v[0] for v in stationsToExtract.getTuples(
                variable=var))
...
def extractTimeSeriesFromProfiles(runTag, stationsToExtract,
    ...
    for key in stationsToExtract.getTuples():
        ...
        if var == 'elev':
            continue  # nothing to be done for elevations
...
def gatherAndExtract(runTag, dataDir, startTime, endTime, extractFile,
    ...
    try:
        ...
        dirTreeManager.saveDataContainerInTree(
            dcs, rule=rule, dtype=np.float32, overwrite=True, compress=True)
    except Exception as e:
    ...
    try:
        ...
        dirTreeManager.saveDataContainerInTree(
            dcs, rule=rule, dtype=np.float32, overwrite=True)
    except Exception as e:
