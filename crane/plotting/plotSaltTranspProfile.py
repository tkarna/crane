...
def makeSubPlot( ax, dcs, startTime,endTime ) :
  ...
  for dc in dcs :
    ...
    if var == 'sTransport' :
      ...
      lStyle = '--'
      lWidth = 1.0
    dia.addSample( dc, label, color=colorDict[tag], linestyle=lStyle, linewidth=lWidth)
  dia.showLegend(**legendProp)
  start_str = startTime.strftime( '%b %d' )
  end_str = endTime.strftime( '%b %d' )
  dia.addTitle(' '.join([loc.upper(),start_str,'-',end_str]) )#, size='large')
...
def computeAndPlotSaltTransport(tags,locs,imgDir,startTime,endTime):
  ...
  # generate data, store in dict: data['stationName']=[total,mean,tidalTransp]
  data = {}
  for loc in locs :
    dcs = []
    for tag in tags :
  ...
  for ax,loc in zip(axList,locs) :
    ...
    if nPlots>1 : addChar( ax, chIter.next() )
...
def parseCommandLine() :
  ...
  parser.add_option('-a', '--stations', action='store', type='string',
                      dest='stations', help='list of stations to plot: jetta,saturn01')
  parser.add_option('-s', '--start', action='store', type='string',
                      dest='startStr', help='Start time of temporal average')
  parser.add_option('-d', '--n-days', action='store', type='float',default=2.0,
                      dest='days', help='Number of tidal days over which temporal average is computed (default:%default)')
