...
def updateXAxis(
    ...
    if xIsTime:
        ...
        if span <= 1.0:
            ...
            meanTime = timeArray.epochToDatetime(
                convertPlotToEpochTime(np.mean(timerange)))
    ...
    def showColorBar(self, **kwargs):
        ...
        if self.ax:
            ...
            cax = divider.append_axes(
                "right", size=0.3, pad=0.1, add_to_figure=False)
        ...
        if self.ax and self.cax:
            ...
            if ticks is not None:
                ...
                loc = matplotlib.ticker.MaxNLocator(
                    nbins=maxticks, prune=prune, integer=integer, symmetric=symmetric)
                self.cb.locator = loc
                self.cb.update_ticks()
...
class stackPlotBase(object):
    ...
    def addPlot(self, plot, tag=None):
        ...
        newHeight = len(
            self.tags) * self.plotheight + self.topPad + self.bottomPad
        ...
        if n == 0:
            ...
            if self.sharex:
                newAx = self.fig.add_subplot(
                    n + 1, 1, n + 1, sharex=self.axGrid[0])
            else:
                newAx = self.fig.add_subplot(n + 1, 1, n + 1)
        # add new axes
