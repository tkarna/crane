...
class stationExtremaPlot(object) :
  ...
  def createAxes(self, fig, rect=111) :
    ...
    tickLabels = np.array([ sta.replace('saturn','sat') for sta in self.stationCoords.keys() ])
  ...
  def plotSample( self, label, station, sample, **kwargs ) :
    ...
    if not label in self.labelStrings :
  ...
  def showLegend(self, **kwargs) :
    ...
    if self.ax :
      self.ax.legend(self.labelSymbols,self.labelStrings, numpoints=1, **kwargs)
...
if __name__=='__main__':
  ...
  dia = stationExtremaPlot('Elevation',stationCoords, unit='m', markersize=20)
  dataset = dict()
