...
class Plots(object):
    ...
    def makeSaltIntrusion(
        ...
        for location in locList:
            ...
            varTitle, varStr, sT, eT = self.getAnnotationStrings(
                var, varPrefix)
            kw = dict(kwargs)
            if not 'title' in kw:
                kw['title'] = ('%s %s\n %s - %s (PST)' %
            ...
            dia = timeSeriesPlot.timeSeriesPlotDC(
                varTitle, unit=UNITS[var], ylim=ylim, **kw)
    ...
    def makeTimeSeries(
        ...
        comKeys = self.coll.getComparableKeys(
            dataType='timeseries', requireObs=False)
        for entry, obsKey, modKeys in comKeys:
            ...
            # ----- Make image -----
            if obsKey:
                dia = timeSeriesPlot.timeSeriesComboPlotDC(
                    varTitle, o, 'Obs', unit=UNITS[var],
                    ylim=yl, err_ylim=erryl, **kw)
            else:
                fig = plt.figure(figsize=(7.5, 3))
                dia = timeSeriesPlot.timeSeriesPlotDC(
                    varTitle, unit=UNITS[var], ylim=yl, **kw)
            for modKey in modKeys:
                m = self.coll.getSample(**modKey)
                dia.addSample(
                    m, color=modelColors[
                        modKey['tag']], label=modKey['tag'])
            dia.makePlot()
    ...
    def makeTimeSeriesStack(
        ...
        for var in self.coll.getAttributes('variable', dataType='timeseries'):
            ...
            stations = list(set(self.coll.getAttributes(
                'location', variable=var)).intersection(set(stationX.keys())))
            orderedSta = sorted(stations, key=lambda k: stationX[k])
            for sta in orderedSta:
                comKeys = self.coll.getComparableKeys(
                    dataType='timeseries', variable=var, location=sta, requireObs=False)
                for entry, obsKey, modKeys in comKeys:
                    station, var, msldepth = entry

                    # dia.addSubplot( sta, VARS[var], UNITS[var] ) # ylabel = var+unit
                    # dia.addSubplot( sta, sta.capitalize(), '' ) # ylabel =
                    # sta
                    dia.addSubplot(
                        sta,
                        sta.upper() + ' ' + msldepth,
                    ...
                    for modKey in modKeys:
                        ...
                        if obsKey and plotError:
                            try:
                                ...
                                dia.addSample(
                                    sta, err, color=errColor, label='err ' + modKey['tag'])
    ...
    def makeStationExtremaPlot(
        ...
        for var in self.coll.getAttributes('variable'):
            dia = stationExtremaPlot.stationExtremaPlotDC(
                VARS[var], stationCoords, unit=UNITS[var], **kwargs)
            comKeys = self.coll.getComparableKeys(
                dataType='timeseries', requireObs=False)
            for entry, obsKey, modKeys in comKeys:
                station, var, msldepth = entry
    ...
    def makeErrorHistograms(
        ...
        # create one plot for each (observation, models) pair
        comKeys = self.coll.getComparableKeys(
            dataType='timeseries', requireObs=True)
        for entry, obsKey, modKeys in comKeys:
            station, var, msldepth = entry
            ...
            # ----- Make plot -----
            dia = errorHistogram.errorHistogramDC(
                o, unit=UNITS[var], label='Obs', **kwargs)
            for modKey in modKeys:
                m = self.coll.getSample(**modKey)
                dia.addSample(
                    m,
...
#      stations = self.coll.getAttributes( 'location',variable=var )
            ...
            for sta in orderedSta:
                comKeys = self.coll.getComparableKeys(
                    dataType='timeseries', variable=var, location=sta, requireObs=True)
                for entry, obsKey, modKeys in comKeys:
                    station, var, msldepth = entry
                    o = self.coll.getSample(**obsKey)
                    if len(o.time) < 3:
                    ...
                    # dia.addSubplot( sta, sta.upper()+' '+msldepth, '' ) #
                    # ylabel = sta msldepth
                    dia.addPlot(
                        sta +
                        msldepth,
                    ...
                    for modKey in modKeys:
                        m = self.coll.getSample(**modKey)
                        dia.addSample(
                            sta + msldepth, m, modKey['tag'],
                            color=modelColors[modKey['tag']])
            if dia.isEmpty():
    ...
    def makeErrorSpectralPlots(
        ...
        for entry, obsKey, modKeys in comKeys:
            ...
            varTitle, varStr, sT, eT = self.getAnnotationStrings(
                var, varPrefix)
            title = (
                'Error spectrum: %s %s Depth: %.1f m Dates: %s - %s (PST)' %
                (station.upper(), varTitle, depth, sT, eT))
            dia.addTitle(title, size=12)
    ...
    def makeHarmonicAnalysisErrorPlots(
            self,
            stationX,
            filetype='png',
            fPrefix='haStack',
        ...
        constsToPlot = [
            ...
            'MSf']
        for var in self.coll.getAttributes('variable', dataType='timeseries'):
            dia = tidalConstPlot.stackAmplitudePhasePlot(
                unit=UNITS[var], **kwargs)
#      stations = self.coll.getAttributes( 'location',variable=var)
            stations = list(set(self.coll.getAttributes(
                'location', variable=var)).intersection(set(stationX.keys())))
            orderedSta = sorted(stations, key=lambda k: stationX[k])
    ...
    def makeTaylorDiagrams(
        ...
        for entry, obsKey, modKeys in comKeys:
            ...
            # ----- Make plot -----
            dia = taylorDiagram.statisticsDiagramDC(
                o, 'Obs', unit=UNITS[var], **kwargs)
            for modKey in modKeys:
                m = self.coll.getSample(**modKey)
                dia.plotSample(
                    m, label=modKey['tag'],
            ...
            for sta in self.coll.getAttributes('location', variable=var):
                ...
                for entry, obsKey, modKeys in comKeys:
                    ...
                    for modKey in modKeys:
                        ...
                        m = self.coll.getSample(**modKey)
                        dia.plotSample(
                            (sta, msldepth), m, label=mLabel, color=col, marker=mar)
    ...
    def makeVertProfileTimeSeriesPlot(
            self,
            filetype='png',
            fPrefix='vprof',
            varPrefix='',
        ...
        for station, var in staVarProf:
            ...
            obsKeys = self.coll.getKeys(
                dataType='timeseries',
                tag=self.coll.getObsTag(),
                location=station,
                variable=var)
            # create plot
            if clim:
            ...
            varTitle, varStr, sT, eT = self.getAnnotationStrings(
                var, varPrefix)
            logScale = True if var in [
                'kine', 'vdff', 'tdff', 'bed_stress'] else False
            climIsLog = True if var in [
                'kine', 'vdff', 'tdff', 'bed_stress'] else False
            dia = profilePlot.stackProfileTimeSeriesDC(
                clabel=varTitle, unit=UNITS[var],
                clim=climVar, logScale=logScale, climIsLog=climIsLog, **kwargs)
            for modKey in modKeys:
                # add model(s)
    ...
    def makeProfilerPlots(
        ...
        comKeys = self.coll.getComparableKeys(
            dataType='profiler', requireObs=False)
        for entry, obsKey, modKeys in comKeys:
            station, var, msldepth = entry
            keys = []
            if obsKey:
            ...
            for key in keys:
                ...
                dia.addTitle(
                    '%s %s Dates: %s - %s (PST)' %
                    (station.upper(), key['tag'], sT, eT), tag=key['tag'])
    ...
    def makeConditionPlot(
        ...
        if cui:
            ...
            dia.addSample(
                'cui', cui, color='b', label='upwelling index',
                ylim=[-1300, 430])
        dia.makePlot()
