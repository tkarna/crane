...
# class that takes the data (transect time series) and plots a snaphot
# (for certain time)
...
def generateTransectFromDataContainer(dc, timeStamp, flipDirection=False):
    ...
    nVert = np.array([len(np.nonzero(xyHashed == uniqueXYHash[i])[0])
                      for i in range(nx)])
    maxNVert = nVert.max()
    nComp = data.shape[1]
...
if __name__ == '__main__':
    ...
    d0 = dataContainer.dataContainer.loadFromNetCDF(
        '~/workspace/cmop/projects/db29dev/transect/run76/data/transect/nchannel_salt_tran_2012-05-01_2012-05-19.nc')
    print d0
