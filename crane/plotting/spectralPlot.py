...
class spectralPlot(object):
    ...
    def updateXAxis(self):
        ...
        sigDigits = 2
        xTicks = [
            round(x, -int(np.floor(np.log10(xmax))) + sigDigits - 1)
            for x in xTicks]
...
class spectralPlotDC(spectralPlot):
    ...
    def addSample(self, sample, **kwargs):
        ...
        spectralPlot.addSample(
            self, sample.time.array, np.squeeze(
                sample.data), **kwargs)
...
if __name__ == '__main__':
    ...
    m2 = 0.80 * np.sin(2 * np.pi * t / TM2 + 0.8) + 0.9 * \
        np.sin(2 * np.pi * t / TS2 + 0.5) - 0.12
    m3 = 0.78 * np.sin(2 * np.pi * t / TM2) + 0.82 * \
        np.sin(2 * np.pi * t / TS2) + 0.03
    ...
    dia.addConstituentIndicators(
        ['M2', 'S2', 'N2', 'L2', 'K2', 'K1', 'O1', 'P1', 'M4', 'M6', 'M8', 'Mf'])
