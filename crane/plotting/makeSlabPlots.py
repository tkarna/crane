    ...
    if stationFileObj is not None:
        for sta in stationFileObj:
            ...
            dia.addStationMarker(
                'all', xSta, ySta, sta.replace(
                    'saturn', 'sat'), printLabel=True, color='k')
...
def makeSlabPlots(
    ...
    for fn in netCDFFiles:
        if startTime and endTime and startTime != endTime:
            dc = meshContainer.meshContainer.loadFromNetCDF(
    ...
    if bBox:
        ...
        aspect = (dcs[0].x.max() - dcs[0].x.min()
                  ) / (dcs[0].y.max() - dcs[0].y.min())
...
def parseCommandLine():
    ...
    usage = (
        'Usage: %prog -s [start date YYYY-MM-DD] -e [end date YYYY-MM-DD] -o [path] -t [stationFile] slabFile\n')
    ...
    parser.add_option(
        ...
        type='string',
        dest='runTag',
        help='Run ID, added in the plot title, e.g. run29 or "selfe k-e" (Default: empty string)')
    parser.add_option(
        '-k',
        '--skip',
        action='store',
    ...
    parser.add_option(
        ...
        type='string',
        dest='bBox',
        help='Limits for x and y, [xmin,xmax,ymin,ymax]')
    parser.add_option(
        '-a',
        '--transect',
        action='store',
        type='string',
        dest='trFiles',
        help='comma separated list of transect bp files to draw in the plot')
    parser.add_option(
        '-M',
        '--colormap',
        action='store',
    ...
    parser.add_option(
        ...
        type='float',
        dest='maxPlotSize',
        help='Max size of the longest edge of individual plot in inches (default %default).',
        default=8)
    parser.add_option(
        '',
        '--fontSize',
    ...
    parser.add_option(
        ...
        type='string',
        dest='bathMeshFile',
        help='A mesh file that contains bathymetry data (for plotting isobath lines)')
    parser.add_option(
        '',
        '--isobaths',
        action='store',
