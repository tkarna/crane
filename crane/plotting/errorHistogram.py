...
class errorHistogramBase(object):
    ...
    def addSample(self, error, label, color=None, weights=None, stats=None):
        ...
        self.dataRange = [min(self.dataRange[0], error.min()), max(
            self.dataRange[1], error.max())]
    ...
    def makePlot(self, **kwargs):
        ...
        # add space on top
        axheight_px = self.ax.bbox.extents[
            3] - self.ax.bbox.extents[1]  # in px
        ...
        if nmseStr != '':
            statsText = u'BIAS: {0:s}{unit:s}\n RMSE: {1:s}{unit:s}\n NMSE: {2:s}'.format(
                meanStr, rmseStr, nmseStr, unit=self.unit)
        else:
            statsText = u'BIAS: {0:s}{unit:s}\n RMSE: {1:s}{unit:s}'.format(
                meanStr, rmseStr, unit=self.unit)
...
class errorHistogramDC(errorHistogramBase):
    ...
    def addSample(self, sample, label, color=None):
        ...
        try:
            ...
            stats = statMod.computeSkillStatistics(
                r.data.ravel(), o.data.ravel())
            errorHistogramBase.addSample(
                self, np.squeeze(err.data),
                label, color, stats=stats)
        except Exception as e:
...
class stackHistogramDC(stackHistogram):
    ...
    def addSample(self, tag, sample, label, color=None):
        ...
        try:
            ...
            stats = statMod.computeSkillStatistics(
                r.data.ravel(), o.data.ravel())
            stackHistogram.addSample(
                self, tag, np.squeeze(err.data),
                label, color, stats=stats)
        except Exception as e:
...
if __name__ == '__main__':
    ...
    dia = stackHistogramDC(
        unit='m', figsize=(10, 10),
        normed=True, shadedLim=[-0.3, 0.3],
        plotMinMax=True)
