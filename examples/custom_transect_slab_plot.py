...
transectDC_salt = dtm.getDataContainer(rule=rule, tag=runTag, dataType='transect',
                                  location='nchannel',variable='salt')
transectDC_kine = dtm.getDataContainer(rule=rule, tag=runTag, dataType='transect',
                                  location='nchannel',variable='kine')
...
# load time series
elevDC = dtm.getDataContainer(rule=rule_monthly, tag=runTag, dataType='timeseries',
                                  location='tpoin',variable='elev',msldepth='0')
...
def makePlot(timeStamp) :
  ...
  # rows 5,6,7, all columns, share x/y axis with previous (sets same ticks etc)
  axTran2 = fig.add_subplot(Grid[4:6, :],sharex=axTran1,sharey=axTran1)

  # gridspec allows to tune the spacing between plots (unit is fraction of font size)
  boundary_pad = 3.5
  horizontal_pad = 0.2
  vertical_pad = 1.0
  bounds = [0,0,1,1] # figure area left,bottom,right,top in normalized coordinates [0,1]
  Grid.tight_layout(fig,pad=boundary_pad,w_pad=horizontal_pad,h_pad=vertical_pad,
                    rect=bounds)
  ...
  dia = slabSnapshotDC(clabel='Salinity',unit='psu', clim=salt_clim, cmap=colormap)
  dia.setAxes(axSlab)
  dia.addSample(slabDC, timeStamp=timeStamp, plotType='contourf',
                bbox=estuarybbox,N=ncontours)
  dia.showColorBar(format='%.2g') # overrides default format for colorbar floats
  ...
  for station in stationsToPlot :
    ...
    dia.addStationMarker(staX,staY,label=station,printLabel=True,marker='*')
  ...
  elevLim = [-1.5,2.5]
  dia = timeSeriesPlotDC2(xlabel=elevMeanTime.strftime('%Y %b %d'),ylim=elevLim)
  dia.setAxes( axElev )
  #dia.addShadedRange( timeStamp, timeStamp+datetime.timedelta(seconds=30), facecolor='IndianRed')
  dia.addShadedRange( timeStamp, timeStamp, edgecolor='IndianRed', facecolor='none', linewidth=2)
  tag = elevDC.getMetaData('tag')
  dia.addSample(elevDC.timeWindow( elevStartTime, elevEndTime ), label=tag,color='k')
  dia.addTitle( 'Elevation ({0:s}) [m]'.format(elevDC.getMetaData('location').upper()) )
  # adjust the number of ticks in x/y axis
  dia.updateXAxis(maxticks=5)
  dia.updateYAxis(maxticks=3,prune='lower')

  # transects
  dia = transectSnapshotDC(clabel='Salinity',unit='psu', cmap=colormap, clim=salt_clim)
  dia.setAxes( axTran1 )
  ...
  saveFigure( imgDir,filename,imgFiletype,verbose=True, dpi=200, bbox_tight=True )
  plt.close()
