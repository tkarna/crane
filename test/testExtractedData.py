...
class compareTransects(comparisonBase) :
  ...
  def testSalt(self):
    ref_files = glob.glob(path+refTag+'/data/transect/nchannel*salt*.nc')
    r = dataContainer.loadFromNetCDF(ref_files[0])
    s = dataContainer.loadFromNetCDF(ref_files[0].replace(refTag,runTag))
    self.compareDCs(r,s)

  def testHvel(self):
    ref_files = glob.glob(path+refTag+'/data/transect/nchannel*hvel*.nc')
    r = dataContainer.loadFromNetCDF(ref_files[0])
    s = dataContainer.loadFromNetCDF(ref_files[0].replace(refTag,runTag))
    self.compareDCs(r,s)

  def testSalt2(self):
    ref_files = glob.glob(path+refTag+'/data/transect/siltransect*salt*.nc')
    r = dataContainer.loadFromNetCDF(ref_files[0])
    s = dataContainer.loadFromNetCDF(ref_files[0].replace(refTag,runTag))
    self.compareDCs(r,s)

  def testHvel2(self):
    ref_files = glob.glob(path+refTag+'/data/transect/siltransect*hvel*.nc')
    r = dataContainer.loadFromNetCDF(ref_files[0])
    s = dataContainer.loadFromNetCDF(ref_files[0].replace(refTag,runTag))
    self.compareDCs(r,s)
...
class compareProfiler(comparisonBase) :
  ...
  def testSalt(self) :
    ref_files = glob.glob(path+refTag+'/data/profiler/saturn01/salt/*.nc')
    r = dataContainer.loadFromNetCDF(ref_files[0])
    s = dataContainer.loadFromNetCDF(ref_files[0].replace(refTag,runTag))
    self.compareDCs(r,s)
...
class compareSIL(comparisonBase) :
  ...
  def testSIL1(self) :
    ref_files = glob.glob(path+refTag+'/data/sil/mainChannel/sil_1/*.nc')
    r = dataContainer.loadFromNetCDF(ref_files[0])
    s = dataContainer.loadFromNetCDF(ref_files[0].replace(refTag,runTag))
    self.compareDCs(r,s)

  def testMaxSIL1(self) :
    ref_files = glob.glob(path+refTag+'/data/sil/mainChannel/max_sil_1/*.nc')
    r = dataContainer.loadFromNetCDF(ref_files[0])
    s = dataContainer.loadFromNetCDF(ref_files[0].replace(refTag,runTag))
    self.compareDCs(r,s)
...
class compareSkillProducts(comparisonBase) :
  ...
  def testAllTimeSeries(self):
    compKeys = self.sc.getComparableKeys( dataType='timeseries',requireMod=True )
  ...
  def testAllProfiles(self):
    compKeys = self.sc.getComparableKeys( dataType='profile',requireMod=True )
    for entry,refKey,modKeys in compKeys :
      r = self.sc.getSample(**refKey)
      s = self.sc.getSample(**modKeys[0])
      self.compareDCs(r,s)
