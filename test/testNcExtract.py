...
class TestTimeSeriesExtraction(unittest.TestCase) :
  ...
  def testElev(self):
    ...
    dcs = se.extractTimeSeries(st,et,'elev',staX,staY,staNames,staZ=staZ)
    self.assertEqual(len(dcs),nGoodSta)

  def testSalt(self):
    se = NCE.selfeExtract(path,var='salt',verbose=verbose)
    dcs = se.extractTimeSeries(st,et,'salt',staX,staY,staNames,staZ=staZ)
    self.assertEqual(len(dcs),nGoodSta)

  def testHvel(self):
    se = NCE.selfeExtract(path,var='hvel',verbose=verbose)
    dcs = se.extractTimeSeries(st,et,'hvel',staX,staY,staNames,staZ=staZ)
    self.assertEqual(len(dcs),nGoodSta)

  def testSalt70(self):
    se = NCE.selfeExtract(path,var='salt',verbose=verbose,fileTypeStr='70')
    dcs = se.extractTimeSeries(st,et,'salt',staX,staY,staNames,staZ=staZ)
    self.assertEqual(len(dcs),nGoodSta)

  def testHvel67(self):
    se = NCE.selfeExtract(path,var='hvel',verbose=verbose,fileTypeStr='67')
    dcs = se.extractTimeSeries(st,et,'hvel',staX,staY,staNames,staZ=staZ)
    self.assertEqual(len(dcs),nGoodSta)
...
class TestProfileExtraction(unittest.TestCase) :
  ...
  def testElevFail(self):
    ...
    with self.assertRaises(Exception) as cm :
      dcs = se.extractVerticalProfile(st,et,'elev',staX,staY,staNames)
...
class TestHLTimeSeriesExtractionXYZ(unittest.TestCase) :
  ...
  def testElevZ(self):
    dcs = NCE.extractForXYZ( path, 'elev', st, et, staX, staY, z=staZ,
                   stationNames=staNames, profile=False, zRelToSurf=False,
                   verbose=verbose)
    self.assertEqual(len(dcs),nGoodSta)

  def testElevZRelSurf(self):
    dcs = NCE.extractForXYZ( path, 'elev', st, et, staX, staY, z=-staZ,
                   stationNames=staNames, profile=False, zRelToSurf=True,
                   verbose=verbose)
    self.assertEqual(len(dcs),nGoodSta)

  def testSaltZ(self):
    dcs = NCE.extractForXYZ( path, 'salt', st, et, staX, staY, z=staZ,
                   stationNames=staNames, profile=False, zRelToSurf=False,
                   verbose=verbose)
    self.assertEqual(len(dcs),nGoodSta)
  ...
  def testSalt70Z(self):
    dcs = NCE.extractForXYZ( path, 'salt.70', st, et, staX, staY, z=staZ,
                   stationNames=staNames, profile=False, zRelToSurf=False,
                   verbose=verbose)
    self.assertEqual(len(dcs),nGoodSta)
  ...
  def testHvelZ(self):
    dcs = NCE.extractForXYZ( path, 'hvel', st, et, staX, staY, z=staZ,
                      stationNames=staNames, profile=False, zRelToSurf=False,
                      verbose=verbose)
    self.assertEqual(len(dcs),nGoodSta)
  ...
  def testHvelZ(self):
    dcs = NCE.extractForXYZ( path, 'hvel.67', st, et, staX, staY, z=staZ,
                      stationNames=staNames, profile=False, zRelToSurf=False,
                      verbose=verbose)
    self.assertEqual(len(dcs),nGoodSta)
...
class TestHLProfileExtractionXYZ(unittest.TestCase) :
  ...
  def testElevFail(self):
    dcs = NCE.extractForXYZ( path, 'elev', st, et, staX, staY, z=staZ,
                      stationNames=staNames, profile=True, zRelToSurf=False,
                      verbose=verbose)
...
class TestHLTransectExtractionForCoords(unittest.TestCase) :
  ...
  def testMultipleAltDiscretization(self):
    dcs = NCE.extractTransectForCoords( staX, staY, path, ['salt.70','hvel.67'],
                                       st, et, 'foo', verbose=verbose )
    self.assertEqual(len(dcs),2)
...
class TestHLSlabExtraction(unittest.TestCase) :
  ...
  def testMultipleK(self):
    dcs = NCE.extractSlabForLevel(path, ['elev','salt','hvel'], st, et, 'foo',
                                  k=1, verbose=True )
  ...
  def testMultipleKAlt(self):
    dcs = NCE.extractSlabForLevel(path, ['salt.70','hvel.67'], st, et, 'foo',
                                  k=1, verbose=True )
    self.assertEqual(len(dcs),2)
  ...
  def testMultipleZRelSurf(self):
    dcs = NCE.extractSlabForLevel(path, ['elev','salt','hvel'], st, et, 'foo',
                                  z=5.5, zRelToSurf=True, verbose=True )
    self.assertEqual(len(dcs),3)
...
if __name__ == '__main__':
  ...
  transectHLSuite = loader.loadTestsFromTestCase(TestHLTransectExtractionForCoords)
