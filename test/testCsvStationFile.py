...
sta3, 5.50000, 2.00000, 0.00000, z, elev"""
  ...
  def testReadFail(self) :
    ...
    self.assertEqual(e.exception.args[0],'Malformed line [\'sta4\', \'5.50000\'] in file tmp_testCsvStationFile/bad.cvs')

  def testReadFail2(self) :
    sf = csvStationFile()
    with self.assertRaises(Exception) as e :
      sf.readFromFile(os.path.join(self.tmpDir,'bad2.cvs'))
    self.assertEqual(e.exception.args[0],'Malformed line [\'sta4\', \'5.50000\', \'ssss\'] in file tmp_testCsvStationFile/bad2.cvs')
...
"""
  ...
  def testAdd(self) :
    ...
    for loc,x,y,z,zType,var in self.toydata :
      sf.addSample( location=loc,x=x,y=y,z=z,zType=zType,variable=var )
    data = sf.getTuples()
    self.assertEqual(data,self.toydata)
  ...
  def testReadFail(self) :
    ...
    with self.assertRaises(Exception) as e :
      sf.readFromFile(os.path.join(self.tmpDir,'bad.cvs'))
    self.assertEqual(e.exception.args[0],'Malformed line [\'sta4\', \'5.50000\', \'2.00000\', \'2.90000\', \'z\'] in file tmp_testCsvStationFileWithDepth/bad.cvs')

  def testReadFail2(self) :
    sf = csvStationFileWithDepth()
    with self.assertRaises(Exception) as e :
      sf.readFromFile(os.path.join(self.tmpDir,'bad2.cvs'))
    self.assertEqual(e.exception.args[0],'Malformed line [\'sta4\', \'5.50000\', \'ssss\', \'2.90000\', \'z\', \'elev\'] in file tmp_testCsvStationFileWithDepth/bad2.cvs')
