#!/usr/bin/python
"""
Extract slab data using the efficient SLIM extract_mod python module.

Examples:

# extract salt,hvel, -d data dir, -t defines transect bp file, -n transect name string, -o output dir, -s -e time range
python slimExtractSlab.py -d /home/tuomas/workspace/cmop/selfe/runs/channel_boxtest/outputs_dihv_spool10/ -v salt,hvel -n test -o tmp -s 2010-6-14 -e 2010-6-14 -z -4
...
def extractSlabForLevel( dataDir, varList, startTime, endTime, name, zCoord=None, kLevel=None, zRelativeToSurf=False, modelCoordSys='spcs' ) :

  mcs = []
  for var in varList :
    ee = extractSlab(dataDir, var, firstStack=1, modelCoordSys=modelCoordSys)
...
def parseCommandLine() :
  ...
  parser.add_option('-n', '--name', action='store', type='string',
                      dest='name', help='name of the slab for identification (default %default)',default='slab')
  ...
  parser.add_option('-o', '--outDirectory', action='store', type='string',
                      dest='outDir', help='base directory for netCDF file tree '
                                                '(optional)')
  parser.add_option('-z', '--zCoord', action='store', type='float',
                      dest='zCoord', help='vertical position of slab: z coordinate (z<0 below datum)')
  parser.add_option('-k', '--kLevel', action='store', type='int',
                      dest='kLevel', help='vertical position of slab: S level, -1 means bottom level, 0 surface')
  parser.add_option('-S', '--zRelToSurf', action='store_true', dest='zRelativeToSurf',
                    help='z coordinate (z>0) is depth below free surface (default %default)',default=False)
  ...
  if zCoord==None and kLevel==None :
    parser.print_help()
  ...
  for i in range(len(varList)):
    dcs = se.extractSlab(startTime, endTime, name, varList[i], zCoord, kLevel, zRelativeToSurf)
