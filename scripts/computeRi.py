...
def interpolateOnGridDC(dc, T, Z):
    ...
    dc2 = dataContainer.dataContainer(
        '', ta, dc.x, dc.y, Z.ravel()[
            None, :], vals.ravel()[
            None, None, :], fn, metaData=meta, acceptNaNs=True)
    return dc2
...
def runningX(dc, T=None, operator=computeRunningMean):
    ...
    meta = dc.getMetaData()
    dc2 = dataContainer.dataContainer(
        '', ta, dc.x, dc.y, z, data, dc.fieldNames[
...
def computeRi(tag, loc):
    ...
    tzmin, zmin = computeRunningMin(
        pc_u.time.asEpoch().array, pc_u.z.ravel(), T=45 * 60.0)
    tzmax, zmax = computeRunningMax(
        pc_u.time.asEpoch().array, pc_u.z.ravel(), T=45 * 60.0)
    ...
    # density gradient
    drhodz = (
        eqState.equationOfStateJackettAlpha(
            s_cnrt,
            t_cnrt,
            p_cntr) *
        dtdz +
        eqState.equationOfStateJackettBeta(
            s_cnrt,
            t_cnrt,
            p_cntr) *
        dsdz)
...
def makeRiPlot(S2DC, N2DC, RiDC, xlim=None, ylim=None):
    ...
    dia.addPlot('ri', clabel='Ri', unit='n/a', logScale=True,
                clim=[-2, 2], climIsLog=True)  # , clim=[0,1] ), clim=[0,0.5] )
    dia.addSample(
        'ri',
        RiDC,
        zorder=2,
    ...
    fn = '_'.join(['riNumber2', RiDC.getMetaData('tag'),
                   RiDC.getMetaData('location'), dateStr])
